...
{
    internal class ListOfListsUnitTest : MonoBehaviour
    {
        public bool Run;
        public bool Log;
        ...
        private void Update()
        {
            ...
            ListOfLists<int> examined = new ListOfLists<int>(Allocator.TempJob);
        ...
        struct FillOperationBufferJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                for (int i = 0; i < AddToListCount; i++)
                {
                    ...
                    {
                        Type = OperationType.AddToList,
                        Val = rnd.NextInt(),
                        ListIndex = rnd.NextInt(0, listCnt)
                    });
        ...
        struct ApplyOperationsToExaminedJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                {
                    Operation op = OperationBuffer[i];
                    if (op.Type == OperationType.AddList)
                    {
                        List.AddList();
                        continue;
                    }

                    if (!List.IsListAllocated(op.ListIndex))
                        List.AllocateList(op.ListIndex);

                    if (op.Type == OperationType.DisposeList)
                    {
                        List.DisposeList(op.ListIndex);
                    }
                    if(op.Type == OperationType.ClearList && List.IsListAllocated(op.ListIndex))
                    {
                        List.ClearList(op.ListIndex);
                    }
                    if(op.Type == OperationType.AddToList && List.IsListAllocated(op.ListIndex))
                    {
        ...
        static void ChangeData(List<List<int>> example, ListOfLists<int> examined)
        {
            for (int i = 0; i < example.Count; i++)
            {
                ...
                if (exampleList == null)
                    continue;
                if (!examined.IsListAllocated(i))
                {
                    UnityEngine.Debug.Log("hele");
                }
                ListOfLists<int>.ListSlice examinedList = examined[i];
                ...
                for(int j = 0; j < exampleList.Count; j++)
                {
                    if (UnityEngine.Random.Range(0, 1f) <= 0.15f)
                    {
                        ...
                        exampleList[j] = newVal;
                        examinedList[j] = newVal;
                    }

                }
            }
        }
        static void TestResult(List<List<int>> example, ListOfLists<int> examined)
        {
            ...
            for (int listIdx = 0; listIdx < example.Count; listIdx++)
            {
                ...
                if (exampleList == null & examined.IsListAllocated(listIdx))
                {
                    UnityEngine.Debug.LogError($"Example inner list is null but examined inner list is still allocated");
                    return;
                }

                if (exampleList != null & !examined.IsListAllocated(listIdx))
                {
                    UnityEngine.Debug.Log(examined.ListInfoBuffer[listIdx].Start + ", " + examined.ListInfoBuffer[listIdx].Length + ", " + examined.ListInfoBuffer[listIdx].Capacity + ", " + examined.ListInfoBuffer[listIdx].AllocId);
                    UnityEngine.Debug.LogError($"Example inner list is not null but examined inner list is disposed");
                    return;
                }
                ...
                ListOfLists<int>.ListSlice examinedList = examined[listIdx];
                for(int i = 0; i < examinedList.Length; i++)
                {
                    int exampleData = exampleList[i];
                    int examinedData = examinedList[i];
